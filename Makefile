...
validate-route53: ## Valida configuração DNS Route 53 (Issue #15)
	...
	@if [ ! -f $(SCRIPTS_DIR)/validate_route53_dns.sh ]; then \
		echo "❌ Script validate_route53_dns.sh não encontrado!"; \
		exit 1; \
	fi
	@chmod +x $(SCRIPTS_DIR)/validate_route53_dns.sh
	@$(SCRIPTS_DIR)/validate_route53_dns.sh
