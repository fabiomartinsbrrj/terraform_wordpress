//criar EIP para cada ZONE - cara public_subnets
...
resource "aws_nat_gateway" "main" {
  ...
  tags = {
    Name = format("%s-%s-nat-gtw", var.project_name, var.public_subnets[count.index].availability_zone)
  }
}
...
resource "aws_route_table" "private" {
  ...
  vpc_id = aws_vpc.main.id
  tags = {
    Name = format("%s-%s-rtb", var.project_name, var.private_subnets[count.index].name)
  }
}
